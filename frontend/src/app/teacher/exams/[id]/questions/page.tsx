...
export default function TeacherExamQuestionsPage() {
  ...
  }, [user, isTeacher, isAdmin, loadExamAndQuestions]);
                    ...
                    <CardContent className="pt-0">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        {JSON.parse(question.options).map((option: string, optIndex: number) => {
                          const isCorrect = question.correctAnswer === String(optIndex);
                          return (
                            <div
                              key={optIndex}
                            ...
                            >
                              <div className="flex items-center gap-2">
                                ...
                                <span className={isCorrect ? 'text-green-900 font-medium' : 'text-gray-900'}>{option}</span>
                                {isCorrect && (
                                  <Badge className="ml-auto bg-green-600 text-white text-xs">Đáp án đúng</Badge>
                                )}
                              </div>
                            </div>
                          );
                    ...
                    <CardContent className="pt-0">
                      <div className="flex gap-4">
                        ...
                        >
                          ...
                          <span className={`font-medium ${question.correctAnswer === 'true' ? 'text-green-700' : 'text-gray-700'}`}>Đúng</span>
                          {question.correctAnswer === 'true' && (
                            <Badge className="ml-2 bg-green-600 text-white text-xs">Đáp án đúng</Badge>
                          )}
                        </div>
                        <div
                          className={`flex items-center gap-2 px-4 py-2 rounded-md border-2 ${
                        ...
                        >
                          ...
                          <span className={`font-medium ${question.correctAnswer === 'false' ? 'text-green-700' : 'text-gray-700'}`}>Sai</span>
                          {question.correctAnswer === 'false' && (
                            <Badge className="ml-2 bg-green-600 text-white text-xs">Đáp án đúng</Badge>
                          )}
                        </div>
          ...
          <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <DialogFooter>
                ...
                <Button onClick={handleCreate} disabled={submitting} className="bg-orange-600 hover:bg-orange-700">
          ...
          <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                ...
                {formData.type === 'MULTIPLE_CHOICE' && (
                  <div className="space-y-3">
                    ...
                    <div className="mt-2">
                      <Label>Đáp án đúng <span className="text-red-500">*</span></Label>
                      <Select 
                        key={`edit-mc-${editingQuestion?.id}-${formData.correctAnswer}`}
                        value={formData.correctAnswer} 
                        onValueChange={(value) => setFormData(prev => ({ ...prev, correctAnswer: value }))}
                      >
                        <SelectTrigger>
                          <SelectValue placeholder="Chọn đáp án đúng">
                            {formData.correctAnswer !== '' && `Đáp án ${String.fromCharCode(65 + parseInt(formData.correctAnswer))}`}
                          </SelectValue>
                        </SelectTrigger>
                        <SelectContent>
                          {formData.options.map((_, index) => (
                ...
                {formData.type === 'TRUE_FALSE' && (
                  <div>
                    <Label>Đáp án đúng <span className="text-red-500">*</span></Label>
                    <Select 
                      key={`edit-tf-${editingQuestion?.id}-${formData.correctAnswer}`}
                      value={formData.correctAnswer} 
                      onValueChange={(value) => setFormData(prev => ({ ...prev, correctAnswer: value }))}
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Chọn đáp án đúng">
                          {formData.correctAnswer === 'true' ? 'Đúng' : formData.correctAnswer === 'false' ? 'Sai' : ''}
                        </SelectValue>
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="true">Đúng</SelectItem>
              ...
              <DialogFooter>
                ...
                <Button onClick={handleUpdate} disabled={submitting} className="bg-orange-600 hover:bg-orange-700">
